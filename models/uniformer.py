...
from train_concat import _parse_args


_,_,feat_dim = _parse_args()
...
class VariablePatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=4, embed_dim=768, stride=None):
        ...
        self.conv_1 = nn.Conv3d(64*8, 64, kernel_size=1, stride=1)
        
        self.norm = nn.LayerNorm(embed_dim)

    def forward(self, x):
        # print ('VariablePatchEmbed forward ...')
        B, C, D, H, W = x.shape
        ...
        ### change channel num
        conv_x_sum = torch.cat((conv_x_sum, conv_x_sum), 1)
...
class UniFormer(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        # initialize feat fusion module
        self.embed_feat_dim = feat_dim
        self.feat_fc_layer = nn.Linear(self.embed_feat_dim, self.embed_dim[-1])

        self.concat_head = nn.Linear(self.embed_dim[-1]*2, num_classes) if num_classes > 0 else nn.Identity()        
        #self.concat_head_add = nn.Linear(self.embed_dim[-1], num_classes) if num_classes > 0 else nn.Identity()
    ...
    def feat_fc(self,feat):
        self.embed_feat_dim = feat.size(-1)
        self.feat_fc_layer = nn.Linear(self.embed_feat_dim, self.embed_dim[-1]).to(feat.device)
        return(self.feat_fc_layer(feat))
...
@register_model
def uniformer_small_IL(num_classes=5, 
                       num_phase=4,
                       pretrained=None, 
                       pretrained_cfg=None,
                       **kwards):
