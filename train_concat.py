...
parser = argparse.ArgumentParser(description='RU-Net 2024')

# Dataset parameters
parser.add_argument('--data_dir', default='', type=str)

parser.add_argument('--feat_csv_file', default='', type=str)
...
def _parse_args():
    ...
    args.train_anno_file = f"./datasets/Data/Label/{args.exp}/{args.fold_name}/fold_{args.fold_index}_train_tumor_label_dl.txt"
    args.val_anno_file = f"./datasets/Data/Label/{args.exp}/{args.fold_name}/fold_{args.fold_index}_val_tumor_label_dl.txt"
    ...
    args.output_dir = f"./output/{args.date}"
    args_text = yaml.safe_dump(args.__dict__, default_flow_style=False)
    
    feat_dim = pd.read_csv(args.feat_csv_file).shape[1] - 2

    return args, args_text, feat_dim
...
def calculate_batch_weights(labels, num_classes):
...
def loss_function(outputs, labels, num_classes):
    batch_weights = calculate_batch_weights(labels, num_classes)
...
def main():
    ...
    print ('img size: ', args.img_size, args.crop_size)
    ...
    if args.smoothing:
        ...
        #基于batch样本数加权
        train_loss_fn = lambda outputs, labels: loss_function(outputs, labels, args.num_classes)
    ...
    if args.rank == 0:
        ...
        output_dir = get_outdir(args.output_dir, args.exp, "train", "fold_"+str(args.fold_index))
    ...
    try:
        ...
        patience = args.patience if hasattr(args, 'patience') else 1500  
        best_loss = float('inf')  
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            #     _logger.info(f"Early stopping at epoch {epoch} due to no improvement in {patience} epochs.")
            #     break
...
def train_one_epoch(
    ...
    for batch_idx, (input, target, feat) in enumerate(loader):
        last_batch = batch_idx == last_idx
        data_time_m.update(time.time() - end)
        
        input, target, feat = input.cuda(), target.cuda(), feat.cuda()
        with amp_autocast():
